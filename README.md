...
Контур боевой, данные настоящие: заявки и отклики видят все участники.

| Контур | Адрес | Логин | Пароль |
|---|---|---|---|
| Приложение водителя | сборка из этого репозитория | `77010030212` | `5555` |
| Кабинет заказчика | [m-truck.atomozen.kz/cabinet](https://m-truck.atomozen.kz/cabinet) | `77020010019` | `5555` |
| Админка MoonShine | [m-truck.atomozen.kz/admin/login](https://m-truck.atomozen.kz/admin/login) | `atom.ozen@gmail.com` | `kukakuka` |

`5555` у водителя и заказчика — не пароль, а фиктивный код из SMS: он подходит
любому номеру, потому что сообщения на тестовом контуре не отправляются.
...
модератором; до этого биржа доступна только для просмотра. Логины для входа —
в разделе «Тестовые доступы» выше.
